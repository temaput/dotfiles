...
call vundle#begin()
    ...
    " --- Python ---
    if has('python')
        Plugin 'Valloric/YouCompleteMe'         "  C/C++/Objective-C and Python code completion. Requires py2
    elseif has('python3')
        " For python3 completion lets use jedi with supertab
...
" set clipboard=unnamed vim перезаписывает системный буфер, это нам не надо
